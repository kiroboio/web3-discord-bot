...
export class Vault {
  ...
  public static getKiroBalance = async({ address, chainId }: {address: string,  chainId: 1 | 4 }) => {
     const kiroContract = Web3Vault.getKiroboTokenContract({ chainId });
     const balance: string = await kiroContract.methods.balanceOf(address).call();
     const vaultAddress = this.contract[address]?.options.address;
     let vaultBalance = "0";
     if(vaultAddress) {
